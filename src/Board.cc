...
{
    for (auto it = m_blocks.cbegin(); it != m_blocks.cend() - 1; ++it)
...
std::string Board::toString() const
{
    std::string result;
    for (auto it = m_blocks.cbegin(); it != m_blocks.cend(); ++it) {
        auto i = (it - m_blocks.cbegin())/m_size;
        auto j = it - m_blocks.cbegin() - i*m_size;
...
Board Board::upperNeighbour(int emptyRow, int emptyColumn) const
{
    ...
    auto newBoard = *this;
    auto prevValue = m_blocks[(emptyRow-1)*m_size + emptyColumn];
...
Board Board::leftNeighbour(int emptyRow, int emptyColumn) const
{
    ...
    auto newBoard = *this;
    auto prevValue = m_blocks[emptyRow*m_size + emptyColumn - 1];
